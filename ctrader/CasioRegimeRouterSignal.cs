...
namespace cAlgo.Robots
{
    ...
    public class CasioRegimeRouterSignal : Robot
    {
        ...
        private void SendSignalEmail(int dir, string routerMode, string technique, double rr, double entry, double stop, double target, int h1Bias, int h4Bias, double h4Adx, double lots, double volumeUnits, int longSupport, int shortSupport)
        {
            ...
            {
                Print("CASIO email skipped: Sender Email or Recipient Email is blank.");
                return;
            }
            ...
            body.AppendLine("Target: " + rr.ToString("0") + "R");
            body.AppendLine("Risk model: " + RiskPercent.ToString("0.##") + "% of current equity");
            ...
            try
            {
                Notifications.SendEmail(SenderEmail, RecipientEmail, subject, body.ToString());
                Print("CASIO email sent: {0}", subject);
            }
            catch (Exception ex)
            {
                Print("CASIO email failed: {0}", ex.Message);
            }
        }
        ...
        private void SendTestEmail()
        {
            ...
            {
                Print("CASIO test email failed: {0}", ex.Message);
            }
        }
