...
from .historical_first_displacement_retrace import Card, prep, setups, replay_pending
            ...
            for da in [0.9,1.2]:
                for bf in [0.70,0.82]:
                    for rt in [0.50,0.705]:
                        for wait in [30,90]:
                            for lb in ([12,20] if fam=="sweep_displacement" else [12]):
                                out.append(Card(
                                    f"SHORT__{fam}__{session}__D{da}__B{bf}__R{rt}__W{wait}__L{lb}",
                                    fam,"h4d1",session,da,bf,rt,wait,lb,0.04
                                ))
    return out
...
def run(data_path,output_dir):
    ...
    for c in cards():
        ...
        rows.append({"strategy":c.name,"family":c.family,"trades":ov["trades"],"wr":ov["wr"],
                     "expectancy_r":ov["expectancy_r"],"pf":ov["pf"],"dd":ov["max_dd_pct"],
                     "end_rm":ov["end_rm"],**ms,"positive_years":int((y.expectancy_r>0).sum()),
                     "pf_years":int((y.pf>1).sum()),"worst_year_exp":float(y.expectancy_r.min()),
    ...
    ranked=rd.sort_values(["strict","dist","positive_years","min_month","wr","expectancy_r"],
    ...
    lines=["# Displacement Shortlist 2017-2020","",
           "Strict target: RR3 / WR>=60 / DD<=25 / min8 every month / 4 of 4 positive years.","",
           f"Cards: {len(rd)}; strict passes: {int(rd.strict.sum())}.","",
...
def main():
    ...
    p.add_argument("--output",default="reports/historical-first-displacement-shortlist")
