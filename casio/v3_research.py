...
def robustness_score(m: dict, folds: list[dict], ys: dict, ms: dict) -> float:
    ...
    score = (
        ...
        + 5 * (ms.get("positive_mode_ratio") or 0)
    )
    return round(float(score), 2)


def monte_carlo_bootstrap(trades: pd.DataFrame, simulations: int = 1000, seed: int = 42) -> tuple[pd.DataFrame, dict]:
...
def run_research(m5: pd.DataFrame, output_dir: str | Path, max_candidates: int = 64,
    ...
    grid = []
    for h4, value, sweep, m5c, rr, session in product(
        [True, False],
        ["ema_atr", "pivot"],
        [1, 2, 3, 4, 5],
        [True, False],
    ...
    for cfg in chosen:
        ...
        cand.append({
            "config_id": _config_id(cfg),
            "robustness_score": robustness_score(m, folds, ys, ms),
            **asdict(cfg),
            **m,
    ...
    _, _, base_dev = evaluate(base)
    _, best_oos_t, best_oos = evaluate(best, cutoff, finish)
    _, _, base_oos = evaluate(base, cutoff, finish)
    ...
    (out / "monte_carlo_summary.json").write_text(json.dumps(_safe(mc_summary), indent=2), encoding="utf-8")
    ...
    def best_tested(question, baseline_variant):
        ...
        r = x.sort_values(["sort", "profit_factor", "trades"], ascending=[False, False, False]).iloc[0].to_dict()
        r.pop("sort", None)
        return _safe(r)
    ...
    summary = {
        ...
        "baseline_rule_set": "v2 regime-first MTF",
    ...
    (out / "best_candidate.json").write_text(
        json.dumps(_safe({"config": asdict(best), "config_id": _config_id(best), "verdict": verdict}), indent=2),
        encoding="utf-8",
    )
    ...
    report = [
        ...
        "",
        "> v3 product; v2 regime-first MTF baseline rules.",
        "",
        f"- Verdict: **{verdict}**",
        ...
        "",
        "The final OOS segment is not used to rank candidates. Monte Carlo is a bootstrap diagnostic, not a forecast. No candidate is auto-deployed.",
    ]
