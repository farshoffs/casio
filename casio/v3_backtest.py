...
def backtest_signals(m5: pd.DataFrame, signals: pd.DataFrame, cfg: V3Config,
    ...
    cols = ["signal_bar", "entry_time", "exit_time", "mode", "direction", "score", "entry", "stop", "target",
            "planned_rr", "gross_r", "cost_r", "net_r", "exit_reason", "regime", "session"]
    if sig.empty:
    ...
    for bar_time, s in sig.iterrows():
        ...
        rows.append({
            "signal_bar": bar_time, "entry_time": entry_time, "exit_time": exit_time, "mode": s["mode"],
            "direction": direction, "score": float(s["score"]), "entry": float(s["entry"]), "stop": stop, "target": target,
            "planned_rr": float(s["rr"]), "gross_r": gross_r, "cost_r": cost_r, "net_r": gross_r - cost_r,
            "exit_reason": reason, "regime": s["regime"], "session": s["session"],
        })
...
def metrics(trades: pd.DataFrame, r_col: str = "net_r") -> dict:
    ...
    return {"trades": int(len(r)), "wins": wins, "losses": losses, "win_rate": float(wins * 100 / len(r)),
            "expectancy_r": float(r.mean()), "profit_factor": float(pf) if pf is not None else None,
...
def year_stability(trades: pd.DataFrame) -> dict:
    ...
    x = trades.copy(); x["year"] = pd.to_datetime(x.entry_time, utc=True).dt.year
    ...
    return {"years": len(exps), "positive_year_ratio": sum(v > 0 for v in exps) / len(exps) if exps else None, "yearly": yearly}
...
def mode_stability(trades: pd.DataFrame) -> dict:
    ...
    return {"positive_mode_ratio": sum(v > 0 for v in exps) / len(exps) if exps else None, "modes": modes}
